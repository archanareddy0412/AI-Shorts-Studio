...
def transcribe_local(media_path: str, language: Optional[str] = None) -> Dict:
    ...
    if cache_path.exists():
        ...
        if cache_mtime >= source_mtime:
            ...
            if not cached["segments"] or cached["duration"] <= 0.0:
                print(
                    f"[transcribe/local] cache is empty/invalid, deleting: {cache_path}",
                    flush=True,
                )
    ...
    print(
        f"[transcribe/local] faster-whisper "
        f"model={LOCAL_WHISPER_MODEL} device={device}",
        flush=True,
    )
    ...
    print(
        "[transcribe/local] STEP A: Loading Whisper model...",
        flush=True,
    )
    ...
    print(
        "[transcribe/local] STEP B: Whisper model loaded successfully",
        flush=True,
    )
    ...
    print(
        "[transcribe/local] STEP C: Starting transcription...",
        flush=True,
    )
    ...
    print(
        "[transcribe/local] STEP D: Transcription iterator created",
        flush=True,
    )
    ...
    print(
        "[transcribe/local] STEP E: Reading transcription segments...",
        flush=True,
    )
    ...
    print(
        ...
        f"{len(segments)} segments, {duration:.0f}s of audio",
        flush=True,
    )
    ...
    print(
        f"[transcribe/local] wrote cache: {cache_path}",
        flush=True,
    )
